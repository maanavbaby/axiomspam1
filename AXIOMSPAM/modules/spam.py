...
from AXIOMSPAM.data import GROUP, PORMS
    ...
    if event.sender_id in SUDO_USERS:
        ...
        try:
            if len(altron) == 3:
                ...
                await event.reply(f"<b><blockquote> Usage:\n  » {hl}spam 13 Maanav Papa\n  » {hl}spam 13 <ʀᴇᴘʟʏ ᴛᴏ ᴛᴇxᴛ>\n\nTo do spam with replying to a user:\n  » {hl}spam 13  <ʀᴇᴘʟʏ ᴛᴏ ᴜꜱᴇʀ></blockquote></b>")
...
async def pspam(event):
    if event.sender_id in SUDO_USERS:
        if event.chat_id in GROUP:
            await event.reply("» ꜱᴏʀʀʏ, ᴛʜɪꜱ ɪꜱ ™°Аксиом ᴘʀᴏᴛᴇᴄᴛᴇᴅ ɢʀᴏᴜᴘ.")
        else:
            try:
...
async def hang(e):
    if e.sender_id in SUDO_USERS:
        if e.chat_id in GROUP:
            await e.reply("» ꜱᴏʀʀʏ, ᴛʜɪꜱ ɪꜱ ™°‌Аксиом ᴘʀᴏᴛᴇᴄᴛᴇᴅ ɢʀᴏᴜᴘ.")
        else:
            try:
